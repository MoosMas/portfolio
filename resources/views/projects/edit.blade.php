...
@section('content')
	<div class="container">
		...
		<form action="{{route('projects.update', $project->id)}}" method="POST" enctype="multipart/form-data" class="w-50 mx-auto">
			@csrf
			@method('PUT')
			...
			<div class="form-group mb-3">
				...
				<textarea name="description" id="description" cols="30" rows="4" class="form-control @error('description') is-invalid @enderror">{{$project->description}}</textarea>
				@error('description')
				<span class="invalid-feedback" role="alert">
                        {{ $message }}
	                </span>
			...
			<div class="form-check form-switch mb-3">
				<input type="checkbox" name="highlight" id="highlight" class="form-check-input" role="switch" value="{{$project->highlight}}">
	...
	<script type="module">
		let projectTags = {!! json_encode(old('tags') ?? $projectTags ?? []) !!};
		let projectTagsIds = projectTags.map((obj) => {
			// obj.text = obj.id; // replace name with the property used for the text
			return obj.id || obj;
		});
		...
		}).val(projectTagsIds).trigger('change');
	</script>
